...
# Test apply_burgess_spacing_correction() ---------------------------------------------

test_that("apply_burgess_spacing_correction() applies correction correctly", {
  ...
  result <- apply_burgess_spacing_correction(
    vh_data = vh_data,
...
test_that("apply_burgess_spacing_correction() can replace original column", {
...
# Test apply_manual_spacing_correction() -------------------------------------
test_that("apply_manual_spacing_correction() works", {
  vh_data <- create_test_vh_data(n = 100, add_zero_periods = TRUE)
  
  # Just a basic test since we're refactoring
  result <- apply_manual_spacing_correction(
    vh_data = vh_data,
    manual_changepoints = as.Date("2024-01-04"),
    sensor_position = "outer",
    method = "HRM"
  )
