      ...
      if (method == "MHR" && !is.na(method_data$calc_window_start_sec[1]) &&
        ...
        p <- p +
          ...
          ggplot2::annotate("text",
                            ...
                            label = "MHR: upstream \u2192 downstream peak",
...
#' Plot Selectable DMA (sDMA) Time Series with Dual Y-Axes
#'
...
#'
#' @param vh_results Results tibble from calc_heat_pulse_velocity()
#' @param sdma_method Name of the sDMA method to plot (e.g., "sDMA:MHR").
...
#' @param quality_filter Logical indicating whether to filter to "OK" quality only.
#'   Default: TRUE
#' @param show_peclet Logical indicating whether to show Peclet number on secondary
#'   y-axis. Default: TRUE
#'
#' @return A ggplot2 object
#'
...
plot_sdma_timeseries <- function(vh_results,
  ...
  # Find all sDMA methods in results
  available_sdma <- unique(vh_results$method[grepl("^sDMA:", vh_results$method)])

  if (length(available_sdma) == 0) {
    stop("No sDMA methods found in results.\n",
         "  Available methods: ", paste(unique(vh_results$method), collapse = ", "), "\n",
         "  Did you calculate sDMA? Use methods = c(\"sDMA:MHR\") or similar.")
  }

  # Select sDMA method
  if (is.null(sdma_method)) {
    if (length(available_sdma) == 1) {
      sdma_method <- available_sdma[1]
      message("Using sDMA method: ", sdma_method)
    } else if (interactive()) {
      cat("\nMultiple sDMA methods found:\n")
      for (i in seq_along(available_sdma)) {
        cat(sprintf("  [%d] %s\n", i, available_sdma[i]))
      }
      cat("\nEnter method number: ")
      method_idx <- as.numeric(readline())
      if (is.na(method_idx) || method_idx < 1 || method_idx > length(available_sdma)) {
        stop("Invalid selection")
      }
      sdma_method <- available_sdma[method_idx]
    } else {
      stop("Multiple sDMA methods found. Please specify sdma_method parameter.\n",
           "  Available: ", paste(available_sdma, collapse = ", "))
    }
  } else {
    # Validate specified method
    if (!sdma_method %in% available_sdma) {
      stop("sDMA method '", sdma_method, "' not found in results.\n",
           "  Available: ", paste(available_sdma, collapse = ", "))
    }
  }

  # Check that sDMA results actually contain required columns
  sdma_data <- vh_results[vh_results$method == sdma_method, ]
  if (nrow(sdma_data) == 0) {
    stop("No data found for sDMA method '", sdma_method, "'")
  }
  if (!"selected_method" %in% names(sdma_data) || all(is.na(sdma_data$selected_method))) {
    stop("sDMA results exist but 'selected_method' column is missing or all NA.\n",
         "  This indicates the sDMA calculation may not have completed properly.")
  }
  if (!"peclet_number" %in% names(sdma_data) || all(is.na(sdma_data$peclet_number))) {
    stop("sDMA results exist but 'peclet_number' column is missing or all NA.\n",
         "  This indicates the sDMA calculation may not have completed properly.")
  }
  ...
  # Interactive prompts for date selection (same as plot_vh_timeseries)
  if (is.null(start_date) && interactive()) {
    cat("\n")
    cat(strrep("=", 67), "\n")
    cat(sprintf("You have %.1f days of data\n", total_days))
  ...
  # Filter data
  selected_position <- sensor_position
  filtered_data <- vh_results %>%
    dplyr::filter(
      datetime >= start_date,
      datetime <= end_date,
      ...
      sensor_position == selected_position,
      !is.na(Vh_cm_hr),
      !is.na(selected_method)  # Only sDMA rows have this
    )
  ...
  # Extract secondary method name from sDMA method
  secondary_name <- sub("^sDMA:", "", sdma_method)

  # Create line segments dataset for colored continuous line
  # Each segment connects consecutive points and is colored by the method at that point
  filtered_data <- filtered_data[order(filtered_data$datetime), ]
  n_points <- nrow(filtered_data)

  # Prepare Peclet scaling BEFORE creating ggplot object
  if (show_peclet) {
    # Calculate scaling factor for secondary axis
    vh_range <- range(filtered_data$Vh_cm_hr, na.rm = TRUE)
    pe_range <- range(filtered_data$peclet_number, na.rm = TRUE)

    # Extend Pe range to ensure Pe=1.0 is visible
    pe_range[1] <- min(pe_range[1], 0)
    pe_range[2] <- max(pe_range[2], 1.5)

    # Scaling: map Pe range to Vh range
    scale_factor <- diff(vh_range) / diff(pe_range)
    offset <- vh_range[1] - pe_range[1] * scale_factor

    # Add Peclet number to data for legend
    filtered_data$peclet_scaled <- filtered_data$peclet_number * scale_factor + offset
  }

  if (n_points > 1) {
    segments_data <- data.frame(
      x = filtered_data$datetime[-n_points],
      y = filtered_data$Vh_cm_hr[-n_points],
      xend = filtered_data$datetime[-1],
      yend = filtered_data$Vh_cm_hr[-1],
      method = filtered_data$selected_method[-n_points]
    )
  } else {
    segments_data <- data.frame()
  }

  # Create base plot (filtered_data now has peclet_scaled if needed)
  p <- ggplot2::ggplot(filtered_data, ggplot2::aes(x = datetime, y = Vh_cm_hr))

  # Add colored line segments if we have data
  ...
  if (show_peclet) {
    p <- p +
      ...
      ggplot2::scale_color_manual(
        values = c("HRM" = "#1F78B4",
                   setNames("#E31A1C", secondary_name),
                   "Peclet Number" = "#666666"),
        name = "Legend"
      ) +
      ggplot2::scale_y_continuous(
        name = expression(paste("V"[h], " (cm hr"^-1, ")")),
        sec.axis = ggplot2::sec_axis(
          trans = ~(. - offset) / scale_factor,
          name = "Peclet Number (Pe)"
        )
      ) +
      ggplot2::annotate("text",
                       x = min(filtered_data$datetime),
                       y = 1.0 * scale_factor + offset,
                       label = "Pe = 1.0 (switching threshold)",
                       hjust = 0, vjust = -0.5, size = 3, color = "black")
  } else {
    p <- p +
      ggplot2::scale_color_manual(
        values = c("HRM" = "#1F78B4", setNames("#E31A1C", secondary_name)),
  ...
  p <- p +
    ggplot2::labs(
      title = paste0("Selectable DMA Time Series: ", sdma_method),
      subtitle = paste0(format(start_date, "%Y-%m-%d"), " to ", format(end_date, "%Y-%m-%d"),
  ...
  cat("\n")
  cat(strrep("=", 67), "\n")
  cat("sDMA PLOT SUMMARY\n")
  cat(strrep("=", 67), "\n")
  cat(sprintf("sDMA method: %s\n", sdma_method))
  cat(sprintf("Date range: %s to %s\n",
              format(start_date, "%Y-%m-%d %H:%M"),
              format(end_date, "%Y-%m-%d %H:%M")))
  cat(sprintf("Duration: %.1f days\n", as.numeric(difftime(end_date, start_date, units = "days"))))
  ...
  # Show method selection breakdown
  cat("\nMethod selection breakdown:\n")
  method_counts <- table(filtered_data$selected_method)
  for (method in names(method_counts)) {
    pct <- 100 * method_counts[method] / nrow(filtered_data)
    cat(sprintf("  %-15s: %s (%.1f%%)\n",
                method,
                format(method_counts[method], big.mark = ","),
                pct))
  }

  # Peclet number statistics
  cat("\nPeclet number statistics:\n")
  cat(sprintf("  Min:    %.3f\n", min(filtered_data$peclet_number, na.rm = TRUE)))
  cat(sprintf("  Median: %.3f\n", median(filtered_data$peclet_number, na.rm = TRUE)))
  cat(sprintf("  Mean:   %.3f\n", mean(filtered_data$peclet_number, na.rm = TRUE)))
  cat(sprintf("  Max:    %.3f\n", max(filtered_data$peclet_number, na.rm = TRUE)))
