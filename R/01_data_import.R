...
#' Robust format detection using first 3000 characters
#'
...
#' Parse a batch of pulse records efficiently
#'
#' @param batch_records Character vector of pulse records
#' @param start_pulse_id Starting pulse ID for this batch
#' @return List with diagnostics and measurements data frames
#' @keywords internal
parse_pulse_batch <- function(batch_records, start_pulse_id) {

  n_records <- length(batch_records)

  # Pre-allocate lists for batch results
  diag_list <- vector("list", n_records)
  meas_list <- vector("list", n_records)

  for (i in seq_along(batch_records)) {
    pulse_id <- start_pulse_id + i - 1
    record_text <- batch_records[i]

    # Quick validation - skip if too short
    if (nchar(record_text) < 50) next

    # Extract datetime - simplified for speed
    datetime_match <- regexec('"date":"([^"]+)"', record_text, perl = TRUE)
    if (datetime_match[[1]][1] == -1) next

    datetime_str <- regmatches(record_text, datetime_match)[[1]][2]

    # Fast datetime parsing - assume ISO format
    pulse_time <- tryCatch(
      as.POSIXct(datetime_str, format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC"),
      error = function(e) as.POSIXct(datetime_str, tz = "UTC")
    )

    if (is.na(pulse_time)) next

    # Extract all numeric values at once
    all_numbers <- as.numeric(stringr::str_extract_all(record_text, "\\d+\\.\\d+")[[1]])

    if (length(all_numbers) < 5) next

    # Create diagnostics row
    diag_list[[i]] <- data.frame(
      pulse_id = pulse_id,
      datetime = pulse_time,
      batt_volt = all_numbers[1],
      batt_current = all_numbers[2],
      batt_temp = all_numbers[3],
      external_volt = if(length(all_numbers) >= 4) all_numbers[4] else NA_real_,
      external_current = if(length(all_numbers) >= 5) all_numbers[5] else NA_real_,
      stringsAsFactors = FALSE
    )
    ...
    if (length(temp_numbers) >= 4) {
      ...
      meas_list[[i]] <- data.frame(
        pulse_id = pulse_id,
        datetime = pulse_time + seq(0, n_measurements - 1),
        do = temp_numbers[idx_base],
        di = temp_numbers[idx_base + 1],
        uo = temp_numbers[idx_base + 2],
        ui = temp_numbers[idx_base + 3],
...
#' Parse a single pulse record (legacy function, kept for compatibility)
#'
#' @param record_text Character string of pulse record
#' @param pulse_id Pulse ID number
#' @return List with diagnostics and measurements
#' @keywords internal
parse_pulse_record <- function(record_text, pulse_id) {

  # Extract datetime - use simpler pattern for speed
  datetime_match <- regexpr('"date"\\s*:\\s*"([^"]+)"', record_text, perl = TRUE)

  if (datetime_match == -1) {
    return(NULL)
  }

  # Extract datetime string directly
  datetime_str <- regmatches(record_text, regexec('"date"\\s*:\\s*"([^"]+)"', record_text, perl = TRUE))[[1]][2]

  # Parse datetime (handle different formats)
  pulse_time <- tryCatch({
    # Extract date components from ISO format or other formats
    if (grepl("T", datetime_str)) {
      # ISO format
      as.POSIXct(datetime_str, format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC")
    } else {
      # Try other formats
    ...
    # Fallback: extract numeric date components
    date_nums <- stringr::str_extract_all(datetime_str, "\\d+")[[1]]
    if (length(date_nums) >= 6) {
      as.POSIXct(paste(
        paste(date_nums[1:3], collapse = "-"),
        paste(date_nums[4:6], collapse = ":")
      ), tz = "UTC")
    } else {
      NA
    }
  ...
  # Extract all numeric values (similar to Gav's approach)
  all_numbers <- stringr::str_extract_all(record_text, "\\d+\\.\\d+")[[1]]
  all_numbers <- as.numeric(all_numbers)

  if (length(all_numbers) < 5) {
    return(NULL)
  }

  # First 5 numbers are diagnostics: bv, bc, bt, ev, ec
  diagnostics <- data.frame(
    pulse_id = pulse_id,
    datetime = pulse_time,
    batt_volt = all_numbers[1],
    batt_current = all_numbers[2],
    batt_temp = all_numbers[3],
    external_volt = if(length(all_numbers) >= 4) all_numbers[4] else NA,
    external_current = if(length(all_numbers) >= 5) all_numbers[5] else NA,
    stringsAsFactors = FALSE
  )

  # Remaining numbers are temperature measurements (groups of 4: do, di, uo, ui)
  temp_numbers <- all_numbers[-(1:5)]

  if (length(temp_numbers) >= 4) {
    n_measurements <- length(temp_numbers) %/% 4

    # Build measurements efficiently using vectorized operations
    # Extract indices for each probe (much faster than loop)
    idx_base <- seq(1, length(temp_numbers) - 3, by = 4)

    measurements <- data.frame(
      pulse_id = pulse_id,
      datetime = pulse_time + seq(0, n_measurements - 1),  # Vectorized datetime
      do = temp_numbers[idx_base],
      di = temp_numbers[idx_base + 1],
      uo = temp_numbers[idx_base + 2],
      ui = temp_numbers[idx_base + 3],
