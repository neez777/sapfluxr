...
#' 1. Unit conversions (g -> kg, cm^3 -> m^3)
...
#' the density ratio: mc = (rho_fw/rho_dw) - 1.
#'
#' @references
...
calculate_wood_properties <- function(wood_properties) {
  ...
  if (method1_complete) {
    ...
    # Use arbitrary volume (1 m^3) for calculations that need weights/volumes
  ...
  # Dry wood density: rho_dw = wd / vf
  deriv$rho_dw_kg_m3 <- deriv$wd_kg / deriv$vf_m3

  # Fresh wood density: rho_fw = wf / vf
  ...
  # Moisture content at FSP: mc_FSP = 0.2 * (rho_dw/rho_s)^(-0.5)
  deriv$mc_FSP_kg_kg <- 0.2 * (deriv$rho_dw_kg_m3 / const$rho_sap_kg_m3)^(-0.5)

  # Specific gravity: G = rho_dw / rho_s
  deriv$specific_gravity <- deriv$rho_dw_kg_m3 / const$rho_sap_kg_m3

  # Void fraction at FSP: Fv_FSP = 1 - G * ((rho_s/rho_cw) + mc_FSP)
  ...
  # Kfw = Ks * (mc - mc_FSP) * (rho_dw/rho_s) + Kdw_FSP
  ...
  # kax = Kfw / (rho_fw * cfw) * 10000  [convert m^2/s to cm^2/s]
  ...
  # Sap flux conversion factor: Z = (rho_dw/rho_s) * ((cdw + mc*cs) / cs)
  # Works for both methods since we now have mc
  ...
  if (method1_complete) {
    ...
    cat("  Fresh volume:", meas$fresh_volume_cm3, "cm^3\n")
  } else {
  ...
  cat(sprintf("  Dry wood density: %.1f kg/m^3\n", deriv$rho_dw_kg_m3))
  if (!is.na(deriv$rho_fw_kg_m3)) {
    cat(sprintf("  Fresh wood density: %.1f kg/m^3\n", deriv$rho_fw_kg_m3))
  }
  cat(sprintf("  Basic density: %.1f kg/m^3\n", deriv$basic_density_kg_m3))
  cat(sprintf("  Specific gravity: %.4f\n", deriv$specific_gravity))
  cat("\n")

  if (!is.na(deriv$thermal_diffusivity_actual_cm2_s)) {
    cat(sprintf("  Actual thermal diffusivity: %.6f cm^2/s\n",
                deriv$thermal_diffusivity_actual_cm2_s))
    cat(sprintf("  Default thermal diffusivity: %.6f cm^2/s\n",
                const$thermal_diffusivity_default_cm2_s))
