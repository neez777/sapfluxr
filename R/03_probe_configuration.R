...
validate_method_requirements <- function(method_name, method_params, config) {
  ...
  if (method_name == "CHMP") {
    # CHMP works best with asymmetric spacing
    if ("upstream" %in% names(config$sensor_positions) &&
      ...
      if (abs(up_dist - down_dist) < 2) {
        warnings <- c(warnings, "CHMP works better with asymmetric sensor spacing")
      }
    }
  }
...
get_method_compatibility_matrix <- function() {
  ...
  compatibility <- data.frame(
    ...
    HRM = c(3, 2, 3, 2),      # HRM works best with symmetric configs
    MHR = c(3, 2, 3, 2),      # Similar to HRM
    CHPM = c(1, 3, 3, 2),     # CHPM works best with asymmetric
    Tmax_Coh = c(2, 2, 3, 2), # T-max methods work reasonably with most
    Tmax_Klu = c(2, 2, 3, 2),
...
# ============================================================================
# NEW YAML-Based Configuration Loading System
# ============================================================================
